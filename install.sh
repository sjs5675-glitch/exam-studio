...
install_codex() {
  ...
  npm i -g @openai/codex@latest || { warn "Codex CLI 설치/업데이트 실패 — 나중에 'npm i -g @openai/codex@latest' 로 재시도하세요."; return 0; }
  hash -r 2>/dev/null || true
  # 이미 로그인돼 있으면 스킵
...
install_claude() {
  ...
  npm i -g @anthropic-ai/claude-code@latest || { warn "Claude Code CLI 설치/업데이트 실패 — 나중에 'npm i -g @anthropic-ai/claude-code@latest' 로 재시도하세요."; return 0; }
  hash -r 2>/dev/null || true
